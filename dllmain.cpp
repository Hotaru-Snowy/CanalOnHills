...
void ApplyMemoryPatch(uintptr_t relativeOffset, size_t size, const char* description) {
    uintptr_t targetAddress = Runtime::GameCoreAddress + relativeOffset;
    DWORD oldProtect;

    if (VirtualProtect((LPVOID)targetAddress, size, PAGE_EXECUTE_READWRITE, &oldProtect)) {
        memset((LPVOID)targetAddress, 0x90, size);
        VirtualProtect((LPVOID)targetAddress, size, oldProtect, &oldProtect);
        std::cout << "[CanalMod] " << description << " applied at 0x" << std::hex << targetAddress << "\n";
    }
    else {
        std::cout << "[CanalMod] Failed to patch " << description << "\n";
    }
}

static void InitHooks() {
    std::cout << "Initializing Canal Mod Hooks...\n";

    // 1. Canal地形检查补丁 (针对Districts_XP2表中的Canal字段的丘陵地形检查)
    // 对应 180362ebd，指令长度 6 字节 (0F 85 A6 02 00 00)
    ApplyMemoryPatch(0x362ebd, 6, "General Canal Hill Check");

    // 2. CanalWonder路径搜索补丁(针对Buildings_XP2表中的CanalWonder字段的丘陵地形检查)
    // 对应 1803639c9，指令长度 6 字节 (0F 85 12 06 00 00)
    ApplyMemoryPatch(0x3639c9, 6, "Panama Canal Pathfinder Check");
}

DWORD WINAPI MainThread(LPVOID lpParam) {
    Runtime::Create(); // 加载真正的 GameCore_XP2_FinalRelease.dll
    if (Runtime::GameCore) {
